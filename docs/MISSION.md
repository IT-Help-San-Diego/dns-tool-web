...
### SPF/DMARC Failure Mode Asymmetry Documented (v26.28.11)
Deep audit of SPF RFC compliance at the same depth as DMARC. Confirmed all SPF posture language, RFC citations (§4.6, §4.6.4, §5.1, §6.1, §10.1 cross-ref with RFC 7489), and CVE references are on solid ground. Key finding documented in the RFC & Security Context panel: SPF and DMARC have asymmetric failure modes for malformed records. SPF with unrecognized mechanisms produces PermError (RFC 7208 §4.6) — the record fails loudly, self-correcting. DMARC with unknown tags silently ignores them (RFC 7489 §11) — the intent vanishes. This asymmetry is now explicitly noted in the SPF RFC context panel to educate domain owners about why DMARC typos are more dangerous than SPF typos. Infrastructure philosophy: modular, tagged, documented code so future engineers can extend without ripping up the floor.

### Unknown DMARC Tag Detection (v26.28.10)
DMARC unknown/unrecognized tag detection implemented per RFC 7489 §11. The IANA DMARC Tag Registry defines the complete set of valid tags: v, p, sp, pct, aspf, adkim, rua, ruf, fo, rf, ri (RFC 7489) plus np, t, psd (DMARCbis). Any tag not in this registry is silently ignored by mail receivers per the RFC — "unknown tags MUST be ignored." This creates a dangerous failure mode: a domain owner publishes `plicy=reject` (typo), thinks they have enforcement, but receivers silently discard the unknown tag and apply no policy. DNS Tool now parses the raw DMARC record, extracts all tag=value pairs, compares tag names against the IANA registry, and flags unrecognized tags with a prominent warning explaining that receivers will silently ignore them. This is a textbook example of the RFC compliance vs operational security distinction: the RFC says ignore it; we say "that's exactly the problem." Template displays unknown tags in a dedicated danger panel with ghost icon and RFC §11 citation. 7 new unit tests cover detection, known tag passthrough, DMARCbis tag recognition, and issue message generation.

### Schema.org Intelligence Pipeline Mapping (v26.28.20)
Rich JSON-LD structured data on indexed pages now maps the full intelligence pipeline to Google's knowledge graph. Index page WebApplication schema includes `featureList` (18 protocol analyzers with RFC citations), `hasPart` (ICIE/ICAE/ICuAE as named SoftwareApplication entities with `@id` identifiers), `isBasedOn` (10 RFC/draft references as CreativeWork — not TechArticle, because Schema.org defines TechArticle as instructional content while RFCs are specifications), and `additionalProperty` (intelligence sources, protocol coverage, output formats, risk classification, CVE coverage). Approach page Article schema maps methodology components with `isBasedOn` RFC references. Live `softwareVersion` injection via Go template variables. Intelligence Pipeline Topology visualization expanded: source nodes, ICIE engine, ICAE/ICuAE auditors, 9 protocol circles, PostgreSQL/Intel Vault storage cylinders, and output hexagons (Reports, JSON API, Schema.org, SVG Badges). The structured data and the topology visualization tell the same story through different channels — one for Google's crawler, one for human eyes.
