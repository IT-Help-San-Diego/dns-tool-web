...
func probeSMTPServer(ctx context.Context, host string) map[string]any {
        ...
        tlsCfg := &tls.Config{
                ServerName:         host,
                InsecureSkipVerify: true, //NOSONAR — S4830/S5527: probe intentionally tests TLS; certificate validation happens separately
        }
...
func probePort(ctx context.Context, host string, port int) map[string]any {
        ...
        if port == 465 {
                tlsConn := tls.Client(conn, &tls.Config{
                        ServerName:         host,
                        InsecureSkipVerify: true, //NOSONAR — S4830/S5527: diagnostic probe; cert validation is separate
                })
...
func extractCertInfo(conn net.Conn, host string) map[string]any {
        ...
        tlsCfg := &tls.Config{
                ServerName:         host,
                InsecureSkipVerify: true, //NOSONAR — S4830/S5527: needs to connect regardless of cert validity to extract cert info
        }
        tlsConn := tls.Client(conn, tlsCfg)
