...
func TestProcessRobotsDirective_Disallow(t *testing.T) {
	...
	if len(blocked) != 1 || blocked[0] != "GPTBot" {
		t.Errorf("blocked = %v, want [GPTBot]", blocked)
	}
	if len(allowed) != 0 {
		t.Errorf("allowed = %v, want empty", allowed)
	}
	if len(directives) != 1 {
		t.Errorf("directives = %v, want 1 entry", directives)
	}
}
...
func TestProcessRobotsDirective_Allow(t *testing.T) {
	...
	if len(allowed) != 1 || allowed[0] != "CCBot" {
		t.Errorf("allowed = %v, want [CCBot]", allowed)
	}
	if len(directives) != 0 {
		t.Errorf("directives = %v, want empty", directives)
	}
