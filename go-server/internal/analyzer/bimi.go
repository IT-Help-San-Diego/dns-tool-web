...
func classifyBIMILogoFormat(contentType string, body []byte, result map[string]any) {
	...
	case strings.Contains(lowerCT, "image"):
		...
		if strings.Contains(content, "<svg") {
			...
			result[mapKeyError] = fmt.Sprintf("BIMI requires SVG Tiny PS format (RFC 9495 §3.1), found %s", formatName)
		}
	default:
		content := strings.ToLower(string(body[:minInt(500, len(body))]))
		if strings.Contains(content, "<svg") {
			...
			result[mapKeyError] = "Not SVG format (RFC 9495 §3.1 requires SVG Tiny PS)"
		}
	}
}
