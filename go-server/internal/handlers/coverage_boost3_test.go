...
func TestTimeAgoFunction(t *testing.T) {
	if got := timeAgo(mustTimeAgo(0)); got != "just now" {
		t.Errorf("expected 'just now', got %q", got)
	}
}

func mustTimeAgo(secondsAgo int) (t0 interface{ Format(string) string }) {
	return nil
}
...
func TestSecurityTrailsErrorMessages(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.errMsg, func(t *testing.T) {
			result := securityTrailsErrorMessage(strings.NewReader(tt.errMsg))
			_ = result
		})
	}
}
...
func TestIpInfoErrorMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.errMsg, func(t *testing.T) {
			result := ipInfoErrorMessage(strings.NewReader(tt.errMsg))
			_ = result
		})
	}
}
...
func TestRiskColorToHexCB3(t *testing.T) {
	if got := riskColorToHex("success"); got != "#4c1" {
		t.Errorf("success = %q", got)
	}
	if got := riskColorToHex("warning"); got != "#dfb317" {
...
func TestBadgeSVGCB3(t *testing.T) {
	...
	svgStr := string(svg)
	if !strings.Contains(svgStr, "DNS Tool") {
		t.Error("expected label in SVG")
	}
	if !strings.Contains(svgStr, "Low Risk") {
		t.Error("expected value in SVG")
	}
	if !strings.Contains(svgStr, "#4c1") {
		t.Error("expected color in SVG")
	}
}

func TestBadgeSVGCovertCB3(t *testing.T) {
	svg := badgeSVGCovert("example.com", "Moderate", "#dfb317")
	if len(svg) == 0 {
		t.Error("expected non-empty SVG")
	}
	svgStr := string(svg)
