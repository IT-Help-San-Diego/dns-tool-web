...
var providerProfiles = map[string]ProviderProfile{
	providerCloudflare: {
		...
		Notes: []ProviderComplianceNote{
			...
			{
				Title:   "Negative cache TTL at 1 hour",
				RFC:     "RFC 2308 §5",
				RFCLink: "https://datatracker.ietf.org/doc/html/rfc2308#section-5",
				Detail:  "Cloudflare's SOA MINIMUM (negative cache TTL) is 3,600 seconds (1 hour). This controls how long resolvers cache NXDOMAIN responses. A newly created DNS record may not be visible for up to 1 hour due to cached negative responses. This is RFC-compliant but aggressive — traditional guidance suggests 300–900 seconds for zones with frequent changes.",
				Verdict: "RFC-compliant, but aggressive for dynamic zones",
			},
		},
	},
	providerRoute53: {
