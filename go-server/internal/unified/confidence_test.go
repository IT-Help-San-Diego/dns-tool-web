...
func TestWeakestLinkIdentification(t *testing.T) {
	tests := []struct {
		...
		{"accuracy weakest", 30, 80, 100, "accuracy"},
		{"currency weakest", 90, 25, 100, "currency"},
		{"maturity weakest", 90, 90, 50, "maturity"},
		{"all equal", 80, 80, 80, "accuracy"},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, _ := identifyWeakestLink(tc.accuracy, tc.currency, tc.ceiling)
			if got != tc.want {
				t.Errorf("identifyWeakestLink(%.0f,%.0f,%.0f) = %s, want %s",
					tc.accuracy, tc.currency, tc.ceiling, got, tc.want)
			}
		})
	}
}
